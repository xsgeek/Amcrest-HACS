...
- This session already runs inside a VS Code native extension. If the target file is already open and focused in the active editor, say so instead of re-invoking the CLI.
- Prefer the PowerShell tool over Bash for the snippet above, since it is native PowerShell rather than POSIX sh.
