...
| --- | --- | --- | --- | --- | --- |
| AD410-001 | Raise a doorbell pressed event that can be used in automations and scripts as a trigger. | Device automation trigger and/or domain event fires when the AD410 button is pressed. | Candidate implemented | Device trigger is available in active HA; needs live button-press test. | Fixed the HA 2026.2 device-trigger import path on branch `AD410-001`. Backend verification on 2026-07-02 shows `doorbell_pressed` is returned for the Main Portal device. Confirm AD410 firmware event code for a real press and verify the preserved `Front Door Bell Pressed` automation fires from it. |
